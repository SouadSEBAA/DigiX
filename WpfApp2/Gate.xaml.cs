...
namespace WpfApp2
{
    ...
    /// </summary>
    public partial class Gate : System.Windows.Controls.UserControl
    {
        protected string data;
        public Outils outil;
        public List<Wire> wires; //Remarque to talk with Nora about this 
        ...
        public void SupprimI()
        {
            InputOutputs = new List<InputOutput>();

            outil.setnb_entrees(outil.getnbrentrees() - 1);

            MAJ_Path(outil.getnbrentrees(), outil.getnbrsoryies());

            LeftGate.Children.Clear();
            LeftGate.ColumnDefinitions.Clear();

            for (int nbr = 0; nbr < outil.getnbrentrees(); nbr++)
            {
                if (outil.getListeentrees()[nbr].GetDisposition() == Disposition.left)
                {
                    AjouterIO(LeftGate, outil.getListeentrees()[nbr], nbr);
                }
            }
        }
        public void SupprimO()
        {
            InputOutputs = new List<InputOutput>();

            outil.setnb_sorties(outil.getnbrsoryies() - 1);

            MAJ_Path(outil.getnbrentrees(), outil.getnbrsoryies());

            RightGate.Children.Clear();
            RightGate.ColumnDefinitions.Clear();

            for (int nbr = 0; nbr < outil.getnbrsoryies(); nbr++)
            {
                if (outil.getListesorties()[nbr].GetDisposition() == Disposition.right)
                {
                    AjouterIO(RightGate, outil.getListesorties()[nbr], nbr);
                }
            }
        }
        //Drag Drop 
        ...
        public void MAJ_Path(int entrees, int sorties)
        {
            int max = Math.Max(entrees, sorties);
            if (outil.GetType().Name == "AddNbits") { path.Width = max * 20; OutilShape.Width = max * 2 + 20; }
            else { path.Height = max * 20; OutilShape.Height = max * 2 + 20; }
        }
        ...
        private void AjouterEntrée(object sender, RoutedEventArgs e)
        {
            ...
            if (type.Name == "Encodeur")
            {
                ...
                Sortie sortie = new Sortie(1, Disposition.right, false, new List<OutStruct>());
            ...
            if (type.Name == "AddNbits")
            {
                if (outil.getnbrentrees() < 10)
                {
                    ...
                    Sortie sortie = new Sortie(1, Disposition.down, false, new List<OutStruct>());
                    outil.AjoutSortie(sortie);
                    AjouterIO(BottomGate, sortie, outil.getnbrsoryies() - 1);
                }
            }
        ...
        private void SupprimerEntrée(object sender, RoutedEventArgs e)
        {
            ...
            if (type.Name == "Encodeur")
            {
                if (outil.getnbrentrees() == 4) { SupprimI(); SupprimI(); SupprimO(); }
                if (outil.getnbrentrees() == 8) { SupprimI(); SupprimI(); SupprimI(); SupprimI(); SupprimO(); }
            }
            if (type.Name == "Decodeur")
            {
                if (outil.getnbrentrees() == 2) { SupprimO(); SupprimO(); SupprimI(); }
                if (outil.getnbrentrees() == 3) { SupprimO(); SupprimO(); SupprimO(); SupprimO(); SupprimI(); }
            }
        }
    ...
    class Encod : Gate
    {
